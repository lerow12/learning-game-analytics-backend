...
def deleteDatabase(host, user, password, database_name):
    init_database = mysql.connector.connect(host=host, user=user, password=password)
...
def createTables(host, user, password, database_name):
    if not initflag:
        print("Initialize the database before use.")
        return
    init_database = mysql.connector.connect(host=host, user=user, password=password, database=database_name)
    init_db_cursor = init_database.cursor()
    query_list = [dbs.Create_BoardDif, dbs.Create_CardDif, dbs.Create_GameTable, dbs.Create_Player_Events]
